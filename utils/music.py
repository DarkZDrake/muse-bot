...
class GuildMusicState:
    ...
    async def play_next_song(self, song=None, error=None):
        ...
        if len(self.voice_client.channel.members)-1 is 0 :
            await self.stop()
...
class Music:
    ...
    async def status(self, ctx):
        ...
        if ctx.music_state.is_playing():
            ...
            await ctx.send(f"Playing **{song.info['title']}**. Volume at {song.volume * 100}% in {ctx.voice_client.channel.mention}")
        else:
    ...
    async def play(self, ctx, *, request: str):
        ...
        if not pathlib.Path(request).is_file():
    ...
    @commands.command()
    async def banzai(self, ctx, singer :int = None):
        """Pseudo Roulette Russa.
        Muse    =   0
        Honoka  =   1
        Umi     =   2
        Kotori  =   3
